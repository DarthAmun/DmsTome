<template>
  <div class="shell">
    <nav class="icon-rail">
      ...
      <button v-for="loc in locations" :key="loc.id" class="rail-icon-btn"
        :class="{ active: activeLocationId === loc.id }" :title="loc.name" @click="selectLocation(loc.id)">
    ...
    <div class="shell-body">
      ...
      <div v-else class="main-canvas">
        ...
        <div v-if="locations.length > 0" class="loc-grid">
          <div v-for="loc in locations" :key="loc.id" class="loc-card v6-card" @click="openLocation(loc)">
...
.status--discovered {
  background: rgba(124, 196, 78, 0.25);
  color: #7cc44e;
}
