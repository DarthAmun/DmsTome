<template>
  <div class="dms-root" :class="{ resizing: !!resizeState }" @keydown.esc="navigateTo(`/campaign/${id}`)">
    ...
    <div class="dms-top">
      ...
      <button class="dms-top-btn" @click="showGrid = !showGrid" :title="showGrid ? 'Hide grid' : 'Show grid'">{{ showGrid ? '▦' : '▢' }} Grid</button>
    ...
    <div class="dms-body">
      ...
      <!-- Widget Canvas -->
      <div :class="['dms-canvas', { 'show-grid': showGrid }]"
        ...
        <div ref="gridRef" :class="['dms-grid', `density-${density}`, { 'drag-active': isDragFromPalette }]">
          ...
          <div v-for="w in widgets" :key="w.id"
            :class="['dms-w', { dragging: dragState?.fromWidgetId === w.id, 'drop-target': dropTargetId === w.id }]"
            :style="{ gridColumn: `span ${w.cols}`, gridRow: `span ${w.rows}` }"
            @dragover.prevent.stop="dropTargetId = w.id"
            @dragleave.self="dropTargetId = null"
            @drop.prevent.stop="onWidgetDrop(w.id)">
  ...
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed) && parsed.length) {
        widgets.value = parsed.map((w: any) => {
          ...
          // Migrate old dice-search kind to dice
...
function buildDefaultLayout(): DmWidget[] {
  ...
  const layout: DmWidget[] = []
  const byType = (t: string) => ents.filter(e => e.type === t)

  const lastSession = byType('session').at(-1)
  if (lastSession) layout.push({ id: `w-s-${lastSession.id}`, kind: 'session', entityId: lastSession.id, cols: 6, rows: 2 })

  layout.push({ id: 'w-dice', kind: 'dice', cols: 3, rows: 3 })
  layout.push({ id: 'w-enc', kind: 'encounter-link', cols: 3, rows: 2 })

  byType('npc').slice(0, 2).forEach(n => layout.push({ id: `w-n-${n.id}`, kind: 'npc', entityId: n.id, cols: 3, rows: 1 }))

  const activeQuest = byType('quest').find(q => (q.attributes as any)?.status === 'active')
  if (activeQuest) layout.push({ id: `w-q-${activeQuest.id}`, kind: 'quest', entityId: activeQuest.id, cols: 3, rows: 1 })

  layout.push({ id: 'w-cond', kind: 'conditions-grid', cols: 3, rows: 2 })
  layout.push({ id: 'w-scratch', kind: 'scratchpad', cols: 6, rows: 1 })
...
function addWidget(opts: { kind: WidgetKind; entityId?: number; recordId?: number }) {
  ...
  if (SINGLETONS.includes(kind) && widgets.value.some(w => w.kind === kind)) return
  if (entityId != null && widgets.value.some(w => w.kind === kind && w.entityId === entityId)) return
  if (recordId != null && widgets.value.some(w => w.kind === kind && w.recordId === recordId)) return

  const id_str = entityId != null ? `w-${kind}-${entityId}`
    : recordId != null ? `w-${kind}-${recordId}`
    : `w-${kind}-${Date.now()}`

  const [cols, rows] = DEFAULT_COLS_ROWS[kind]
  widgets.value.push({ id: id_str, kind, entityId, recordId, cols, rows })
}
...
function onResizeMove(e: MouseEvent) {
  ...
  if (!s) return
  const newCols = Math.max(2, Math.min(12, s.startCols + Math.round((e.clientX - s.startX) / (s.cellW + s.gap))))
  const newRows = Math.max(1, Math.min(8,  s.startRows + Math.round((e.clientY - s.startY) / (s.cellH + s.gap))))
  const w = widgets.value.find(w => w.id === s.widgetId)
  if (!w || (w.cols === newCols && w.rows === newRows)) return
  widgets.value = widgets.value.map(w => w.id === s.widgetId ? { ...w, cols: newCols, rows: newRows } : w)
}
...
const isDragFromPalette = computed(() => !!dragState.value && !dragState.value.fromWidgetId)

function onPalDragStart(e: DragEvent, kind: WidgetKind, entityId?: number, recordId?: number) {
  dragState.value = { kind, entityId, recordId }
  if (e.dataTransfer) e.dataTransfer.effectAllowed = 'copy'
}

function onWidgetDragStart(e: DragEvent, w: DmWidget) {
  dragState.value = { kind: w.kind, entityId: w.entityId, recordId: w.recordId, fromWidgetId: w.id }
  if (e.dataTransfer) e.dataTransfer.effectAllowed = 'move'
}

function onCanvasDragOver(e: DragEvent) {
  if (!dragState.value) return
  if (e.dataTransfer) e.dataTransfer.dropEffect = dragState.value.fromWidgetId ? 'move' : 'copy'
}

function onCanvasDrop() {
  if (!dragState.value) return
  if (dragState.value.fromWidgetId) {
    moveWidget(dragState.value.fromWidgetId, null)
  } else {
    addWidget({ kind: dragState.value.kind, entityId: dragState.value.entityId, recordId: dragState.value.recordId })
  }
...
function onWidgetDrop(targetId: string) {
  if (!dragState.value) return
  if (dragState.value.fromWidgetId && dragState.value.fromWidgetId !== targetId) {
    moveWidget(dragState.value.fromWidgetId, targetId)
  } else if (!dragState.value.fromWidgetId) {
    addWidget({ kind: dragState.value.kind, entityId: dragState.value.entityId, recordId: dragState.value.recordId })
  }
  dragState.value = null
  dropTargetId.value = null
...
function moveWidget(srcId: string, beforeId: string | null) {
  const i = widgets.value.findIndex(w => w.id === srcId)
  if (i < 0) return
  const src = widgets.value[i]
  const rest = widgets.value.filter(w => w.id !== srcId)
  if (!beforeId) {
    widgets.value = [...rest, src]
    return
  }
  const j = rest.findIndex(w => w.id === beforeId)
  if (j < 0) { widgets.value = [...rest, src]; return }
  widgets.value = [...rest.slice(0, j), src, ...rest.slice(j)]
}
</script>
...
.dms-canvas.show-grid {
  background-image:
    linear-gradient(var(--border) 1px, transparent 1px),
    linear-gradient(90deg, var(--border) 1px, transparent 1px);
  background-size: calc((100% - 36px) / 12) var(--row-h, 110px);
...
.dms-w.dragging { opacity: 0.4; transform: scale(0.97); }
.dms-w.drop-target { border-color: color-mix(in oklch, var(--accent) 60%, transparent); box-shadow: 0 0 0 2px var(--accent-bg); }
