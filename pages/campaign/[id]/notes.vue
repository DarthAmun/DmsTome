<template>
  <div class="shell">
    <nav class="icon-rail">
      ...
      <button v-for="tab in typeTabs" :key="tab.type" class="rail-icon-btn" :class="{ active: activeType === tab.type }"
        :title="tab.plural" :style="activeType === tab.type ? { color: tab.color, background: tab.color + '22' } : {}"
        @click="activeType = tab.type; selectedId = null">
    ...
    <div class="shell-body">
      <header class="top-bar">
        ...
        <!-- Graph toggle -->
        <Button :severity="showGraph ? undefined : 'secondary'" @click="showGraph = !showGraph; selectedId = null">
          <template #icon>
      ...
      <div v-else class="main-canvas">
        ...
        <div v-if="sortedEntities.length > 0" class="cards-grid">
          <EntityCard v-for="e in sortedEntities" :key="e.id" :entity="e" @click="selectedId = e.id"
            @edit="selectedId = e.id" @duplicate="store.duplicateEntity(e.id)" @delete="confirmDelete(e)" />
        </div>
...
const route = useRoute(); const store = useNotesStore()
const campaignId = Number(route.params.id)
const campaignName = ref('Campaign'); const selectedId = ref<number | null>(null)
const activeType = ref<EntityType>('note'); const search = ref(''); const sortBy = ref('updated'); const showGraph = ref(false)
...
const sortOptions = [{ label: 'Last edited', value: 'updated' }, { label: 'Created', value: 'created' }, { label: 'Name', value: 'name' }]

const sortedEntities = computed(() => {
  const list = (store.byType[activeType.value] ?? []).filter(e => !search.value || e.name.toLowerCase().includes(search.value.toLowerCase()))
...
onMounted(async () => {
  ...
  if (window.dmforge) { const camps = await window.dmforge.campaigns.list(); campaignName.value = camps.find((c: any) => c.id === campaignId)?.name ?? 'Campaign' }
  if (route.query.id) selectedId.value = Number(route.query.id)
  if (route.query.type) activeType.value = route.query.type as EntityType
})
...
function navigateByTypeAndName(type: string, name: string) {
  ...
  if (e) { activeType.value = e.type; selectedId.value = e.id; showGraph.value = false }
  else if (confirm(`"${name}" doesn't exist yet. Create it?`)) {
    store.createEntity(campaignId, type as EntityType, name).then(en => { activeType.value = en.type; selectedId.value = en.id; showGraph.value = false })
  }
}
async function confirmDelete(entity: any) { if (confirm(`Delete "${entity.name}"?`)) await store.deleteEntity(entity.id) }
</script>
