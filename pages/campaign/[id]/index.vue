<template>
  <div class="camp-overview screen-in">
    ...
    >
      ...
      <div class="camp-banner-fade" />
      <!-- Banner edit controls -->
      <div class="camp-banner-actions">
        <label class="camp-banner-btn" title="Upload banner image">
          <input type="file" accept="image/*" style="display:none" @change="uploadBanner" />
          ↑ Banner
        </label>
        <button v-if="bannerSource" class="camp-banner-btn camp-banner-btn--clear" title="Remove banner" @click="clearBanner">✕</button>
      </div>
    </div>
    ...
    <div class="camp-body">
      ...
      <!-- Stats grid -->
      <div class="camp-section-label">Chronicle</div>
      ...
      <!-- Quick access -->
      <div class="camp-section-label" style="margin-top:24px">Quick Access</div>
      <div class="camp-chapters">
        <NuxtLink :to="`/campaign/${id}/encounters`" class="camp-chapter">
          <span class="camp-chapter-icon" style="color:#4ab8e8">⚔</span>
          <div class="camp-chapter-body">
            <div class="camp-chapter-name">Encounters</div>
            <div class="camp-chapter-sub">Tactical maps, tokens &amp; fog of war</div>
          </div>
          <span class="camp-chapter-arrow">›</span>
        </NuxtLink>

        <NuxtLink :to="`/campaign/${id}/map`" class="camp-chapter">
          <span class="camp-chapter-icon" style="color:#e8924a">🗺</span>
          <div class="camp-chapter-body">
            <div class="camp-chapter-name">World Map</div>
            <div class="camp-chapter-sub">World maps with pinned locations</div>
          </div>
          <span class="camp-chapter-arrow">›</span>
        </NuxtLink>
      </div>
      ...
      <!-- System link -->
      <div class="camp-section-label" style="margin-top:24px">Settings</div>
      <div class="camp-setting-row">
        <div>
          <div class="camp-setting-title">Linked System</div>
          <div class="camp-setting-hint">Link a rules system to enable stat auto-fill in encounters.</div>
        </div>
        <select class="camp-setting-select" :value="linkedSystemId ?? ''" @change="setLinkedSystem($event)">
...
async function uploadBanner(e: Event) {
  ...
  await dbApi.campaigns.update(id, { banner_source: src, banner_type: 'file' })
}

async function clearBanner() {
  bannerSource.value = null
  await dbApi.campaigns.update(id, { banner_source: null, banner_type: undefined })
}
...
.camp-banner:hover .camp-banner-actions { opacity: 1; }
.camp-banner-btn {
  padding: 4px 10px;
...
.camp-banner-btn:hover { background: oklch(0% 0 0 / 0.55); }
.camp-banner-btn--clear { color: rgba(255,180,160,0.95); }
...
.camp-chapter:hover { background: var(--surface-hi); border-color: var(--border-hi); }
.camp-chapter-icon { font-size: 20px; flex-shrink: 0; }
.camp-chapter-body { flex: 1; min-width: 0; }
.camp-chapter-name { font-size: 14px; font-weight: 600; color: var(--text); }
.camp-chapter-sub { font-size: 12px; color: var(--text3); margin-top: 2px; }
.camp-chapter-arrow { font-size: 20px; color: var(--text3); flex-shrink: 0; transition: transform 0.12s; }
.camp-chapter:hover .camp-chapter-arrow { transform: translateX(3px); }
...
.camp-setting-title { font-size: 13px; font-weight: 600; color: var(--text); }
.camp-setting-hint { font-size: 12px; color: var(--text3); margin-top: 2px; }
.camp-setting-select {
  flex-shrink: 0;
  padding: 6px 10px;
  border-radius: var(--r1);
...
.camp-setting-select:focus { border-color: var(--accent); }
</style>
